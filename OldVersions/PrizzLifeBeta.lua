--[[ PrizzLife V1.2 ReWrite, made by Nivalos, influenced by Vlaair.
...
Update Log 2:
Fixed Gun giving, to a certain extent, I'm planning on adding the new guns.
Pushed a small fix for Team GUI, and hopefully team full detection
Fixed shitty executor detection (thanks to Nyxusss since I don't have a shitty executor)
--]]
...
local function ChangeTeam(TeamName, Switch)
    ...
    elseif TeamName == "Criminals" then
        if game.Players.LocalPlayer.Team == "Guards" then
        TeamEve("Neutral")
        task.wait(4)
        hrp.CFrame = CFrame.new(-974.720, 108.324, 2058.452)
        elseif game.Players.LocalPlayer.Team == "Inmates" then 
        hrp.CFrame = CFrame.new(-974.720, 108.324, 2058.452)
        elseif game.Players.LocalPlayer.Team == "Neutral" then
        TeamEve("Inmates")
        task.wait(0.4)
        hrp.CFrame = CFrame.new(-974.720, 108.324, 2058.452)
        else
...
local function GiveGun(itemname)
	...
	elseif itemname == "M4A1" then
		if ownsAnyPass(game.Players.LocalPlayer.UserId, {96651, 643697197}) then
			...
			Notif("Gamepasses❌", "You don't own this PL gamepass, you can't get this item.", 6)
			return
		end
	end
...
local CrashMethod = function(typeofcrash, args)
	...
	elseif typeofcrash == "crashkill" then
		if LocalPlayer.Team == args.Team then
			if args.Team == "Inmates" then
				ChangeTeam("Criminals", true)
			else
			    ChangeTeam("Inmates", true)
			end; RTPing()
...
--[[Client Handler (disabled for now)
coroutine.wrap(function()
...
--]]
	...
	--[[AddList("CRASH/LAG CMDS", false, true) -- CRASH COMMANDS (these don't work anyways)
...
UnloadScript = function()
    ...
    workspace:FindFirstChild("PLADMIN UNLOADED SUCCESS"):Destroy()
end
...
Notif("Gamepasses✅", "You own one, or more of these PL Gamepasses, you will be able to use the certain weapons from the Gamepasses you have!", 6)
else
Notif("Gamepasses❌", "You don't own any PL Gamepasses, you will not be able to use any special items.", 6)
end
...
if executor == "Solara" or "Xeno" or "JJSploit" then
