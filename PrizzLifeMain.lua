--[[ PrizzLife V1.2 Main ReWrite, made by Nivalos, influenced by Vlaair.
This script is so broken I might have to spend a couple weeks to fix this.
This is still extremely broken, but out of what it was before, if you would like to fork it to contribute, you may.

Update Log 10:
Added more checks for getting guns, so its less likely to bug out.
Fixed Backpack clipping issue!

Update Log 9:
Fixed AllGuns command.
Commented out commands that arent currently working.

Update Log 8:
Fixed Dragging for the main UI.
ADDED DRAGGING FOR THE TOGGLE BUTTON!!!
Added Revert command to reset the GUI, and Toggle button positions.

Update Log 7:
Fixed very small issue where the C4 wouldn't give (wrong function), and added a new alias for it "bomb"

Update Log 6:
Notification System Overhall, and fixed some notification issues.

All guns, and items added

UnLoad command completely fixed, so if you execute PrizzLife again, it won't error you.
Fixed Shitty Executor Detection.
Fixed Team full detection.

Update Log 5:
Made the UI alot smaller, to help with Mobile users!
--]]
...
Toggles_Frame.Size = UDim2.new(0, 295, 0, 270)
...
local ItemCheck = function(itemname)
...
local function ChangeTeam(TeamName, Switch)
    ...
    elseif TeamName == "Criminals" then
        ...
        if player.Team and player.Team.Name == "Guards" then
            ...
            char.HumanoidRootPart.CFrame = CFrame.new(-974.720, 108.324, 2058.452)
            
        elseif player.Team and player.Team.Name == "Inmates" then 
            local char = player.Character or player.CharacterAdded:Wait()
            char.HumanoidRootPart.CFrame = CFrame.new(-974.720, 108.324, 2058.452)
            
        elseif player.Team and player.Team.Name == "Neutral" then
            TeamEve("Inmates")
            local char = player.Character or player.CharacterAdded:Wait()
            char.HumanoidRootPart.CFrame = CFrame.new(-974.720, 108.324, 2058.452)
    ...
    elseif TeamName == "Neutral" then
        ...
        Notif("Team", "Team not found, try again.", 3)
    end
end
-- Teams
...
local function GiveGun(itemname)
	local player = game.Players.LocalPlayer
	...
	if itemname == "RemingtonGuards" then
	  if not ItemCheck("Remington 870") then
    ...
    if not ItemCheck("AK-47") then
		hrp.CFrame = GunLocations.AK47
		task.wait(0.2)
		ItemEve("AK-47", "Meshes/AK47_7")
	...
	elseif itemname == "M4A1" then
		if ownsAnyPass(player.UserId, {96651, 643697197}) then
		  if not ItemCheck("M4A1") then
	...
	elseif itemname == "FAL" then
		if ownsAnyPass(player.UserId, {1443271}) then
		  if not ItemCheck("FAL") then
	...
	elseif itemname == "Sniper" then
		if ownsAnyPass(player.UserId, {699360089}) then
			if player.Team and player.Team.Name == "Guards" then
			  if not ItemCheck("Sniper") then
	...
	elseif itemname == "Revolver" then
		if ownsAnyPass(player.UserId, {699360089}) then
			if player.Team and player.Team.Name == "Guards" then
			  if ItemCheck("Revolver") then
				hrp.CFrame = GunLocations.GuardItems
				task.wait(0.2)
                ItemEvent:InvokeServer(workspace.Prison_ITEMS.giver:GetChildren()[13]:GetChildren()[2])
			...
			elseif player.Team and player.Team.Name == "Criminals" then
			  if not ItemCheck("Revolver") then
				hrp.CFrame = GunLocations.CrimItems
				task.wait(0.2)
			    GiveGun("Revolver", "Meshes/revolver (3)")
			    Notif("OK", "Obtained Revolver.")
			    else
			      Notif("Error", "You already have the Revolver.")
			  end
...
local function GiveItem(itemname)
    ...
    elseif itemname == "RiotHelmet" and game.Players.LocalPlayer.Team.Name == "Guards" then
        ...
        hrp.CFrame = ItemLocations.RiotItems
        task.wait(0.2)
        ItemEvent:InvokeServer(workspace.Prison_ITEMS.hats["Riot helmet"].hat)
    ...
    elseif itemname == "SkiMask" and game.Players.LocalPlayer.Team.Name == "Guards" then
        ...
        hrp.CFrame = ItemLocations.RiotItems
        task.wait(0.2)
        ItemEvent:InvokeServer(workspace.Prison_ITEMS.hats["Ski mask"].hat)
...
-- GunMod
local function GunMods(Type, Gun)

end

local function KillAura(enabled, range)

end


-- MAIN FUNCTIONS LIST END
...
local OpenDoors = function(includeGate)
	local oldteam = nil
	if #Teams.Guards:GetPlayers() >= 8 and not (LocalPlayer.Team == "Guards") then
		Notif("Error", "Guards team full.")
		return
	elseif not (LocalPlayer.Team == "Guards") then
		oldteam = LocalPlayer.Team; SaveCamPos(); ChangeTeam("Guards"); waitfor(LocalPlayer.Character, "HumanoidRootPart", 3)
	...
	if oldteam then
		wait(2); SaveCamPos()
		if oldteam == "Criminals" then
			ChangeTeam("Criminals", true)
		elseif oldteam == "Inmates" then
			ChangeTeam("Inmates", true)
		end
	end
end
...
local TasePL = function(plr, args)
	local oldteam = nil
	if not (LocalPlayer.Team == "Guards") then
	...
	if args == "teams" then
		for _, tea in pairs(plr:GetPlayers()) do
			if (not (tea == LocalPlayer) and not (tea.Team == "Criminals")) and CheckWhitelist(tea) and tea.Character and tea.Character:FindFirstChild("Humanoid") and not (tea.Character:FindFirstChild("Humanoid").Health == 0) then
...
local MakeCrim = function(args, savepos, tpback, ArrestLater)
	if args.Team == "Criminals" then
		...
		repeat
			...
			RTPing(0);RTPing(0)
		until args.Team == "Criminals" or tick() - timeout >= 0; timeout = nil
		if ArrestLater then
			ArrestPL(args)
		end;if tpback then
			...
			while States.AnnoyingPlayer do task.wait()
				...
				pcall(function()
					if plr and plr.Character and plr.Character:FindFirstChildOfClass("Humanoid").Health ~= 0 then
						if plr.Team == "Criminals" or (plr.Team == "Inmates" and GetIllegalReg(plr)) then
						...
						elseif plr.Character and plr.Character:FindFirstChildOfClass("Humanoid") and plr.Character:FindFirstChildOfClass("Humanoid").Health ~= 0 then
							readytoarrest = false
							repeat task.wait() until plr.Character and plr.Character:FindFirstChild("HumanoidRootPart") and not (plr.Team == "Neutral")
							coroutine.wrap(function()
							...
							repeat task.wait()
								...
								if plr.Team == "Neutral" then
									LAction("unsit", true)
									repeat task.wait() until not (plr.Team == "Neutral")
									break
								end
								...
								if plr.Team == "Criminals" or (GetIllegalReg(plr) and plr.Team == "Inmates") then
									break
								end
			...
			if v.Character and not v.Character:FindFirstChildWhichIsA("ForceField") and not (v.Character:FindFirstChild("Humanoid").Health == 0) then
				...
				if v.Team == LocalPlayer.Team then
					if v.Team == "Criminals" then
						ChangeTeam("Criminals", true)
					elseif v.Team == "Criminals" or v.Team == "Guards" then
						neutralkill = true
					end
				end
				for i = 1, 10 do
...
local GiveKeyCard = function(player)
	if player == LocalPlayer then
		if workspace.Prison_ITEMS.single:FindFirstChild("Key card") then
			...
			local oldteam, oldpos = LocalPlayer.Team, LocalPlayer.Character:FindFirstChild("HumanoidRootPart").CFrame
			repeat task.wait()
				if LocalPlayer.Team ~= "Guards" then
					if #Teams.Guards:GetPlayers() > 7 then
						Notif("Error", "Guards team is full! Failed to givekeycard.")
						break
					end; ChangeTeam("Guards", true)
				end; LAction("died")
			...
			if oldteam == "Inmates" then
				ChangeTeam("Inmates", true)
			elseif oldteam == "Criminals" then
				ChangeTeam("Criminals", true)
			end; ItemHand(false, "Key card"); ItemHand(false, "Key card"); ItemHand(false, "Key card")
		end
	else
		local finallykeycard = nil; local oldteam = LocalPlayer.Team
			...
			while wait(.7) do
				pcall(function()
					if Loops.TaseTeams.All then
						if LocalPlayer.Team ~= "Guards" and #Teams.Guards:GetPlayers() < 8 then
							ChangeTeam("Guards", true)
						end; if LocalPlayer.Team.Name == "Guards" then 
						...
						if Loops.TaseTeams.Inmates then
							...
							if LocalPlayer.Team ~= "Guards" and #Teams.Guards:GetPlayers() < 8 then
								ChangeTeam("Guards", true)
							end; if LocalPlayer.Team == "Guards" then 
								TasePL(temp, "tables")
							end
						end; temp = nil
					end
...
local OnCommand = function(text)
	...
	elseif cm("revert") or cm("resetgui") or cm("rvgui") then
	    MainFrame.Position = UDim2.new(0.5, 0, 0.5, 0)
		savedClosedFramePosition = UDim2.new(0.499449551, 0, 0.128650904, 0)
	...
	elseif cm("loopopendoors") or cm("loopdoors") then
		if not Saved.Thread.LoopOpenDoors then
			task.spawn(function()
				while true do
					if not (LocalPlayer.Team == "Guards") then
						SavedArgs.OldTeamOpenDoor = LocalPlayer.Team
						ChangeTeam("Guards", true)
					end
					OpenDoors(); wait(1)
					if not Saved.Thread.LoopOpenDoors then
	...
	elseif cm("unloopopendoors") or cm("unloopdoors") then
		...
		if SavedArgs.OldTeamOpenDoor then
			if SavedArgs.OldTeamOpenDoor == "Inmates" then
				ChangeTeam("Inmates", true)
			elseif SavedArgs.OldTeamOpenDoor == "Criminals" then
				ChangeTeam("Criminals", true)
			end
		end
			...
			task.spawn(function()
				...
				if LocalPlayer.Team ~= plr.Team then
					if plr.Team == "Guards" then
						ChangeTeam("Guards", true)
					elseif plr.Team == "Inmates" then
						ChangeTeam("Inmates", true)
					elseif plr.Team == "Criminals" then
						ChangeTeam("Criminals", true)
					else
						ChangeTeam("Neutral", false)
					end
				end
	...
	elseif cm("inmate") or cm("inmates") or cm("in") then
	    if #Teams.Inmates:GetPlayers() > 6 then
	...
	elseif cm("guard") or cm("guards") or cm("gu") then
		if #Teams.Guards:GetPlayers() > 6 then
			Notif("Error", "Guards team full!")
			return
		end
		ChangeTeam("Guards", true)
	...
	elseif cm("remington") or cm("shotgun") or cm("rem") then
		...
		GiveGun("RemingtonCrim")
		end
	elseif cm("m4") or cm("m4a1") then
			GiveGun("M4A1")
	elseif cm("fal") then
			GiveGun("FAL")
	elseif cm("sniper") or cm("m700") then
			GiveGun("Sniper")
	elseif cm("revolver") or cm("pistol") then
			GiveGun("Revolver")
    elseif cm("c4") or cm("explosive") or cm("boom") or cm("bomb") then
			GiveItem("MafiaC4")
	...
	elseif cm("riotarmor") or cm("armor") then
	    GiveItem("RiotClothes")
	elseif cm("mafiaarmor") or cm("armor2") then
	    GiveItem("MafiaClothes")
	elseif cm("mafiavest") or cm("vest") then
	    GiveItem("ArmorVest")
	...
	elseif cm("discord") or cm('support') or cm('help') then
		if setClipboard then
			toClipboard('https://discord.com/invite/')
			Notif('SUCCESS', 'Copied to clipboard!\ndiscord.gg/')
		else
			Notif('Discord Invite', 'https://discord.gg/SZdAVGqAGs', 10)
			if httprequest then
				httprequest({
		...
		print(1)
		elseif cm("github") or cm("git") then
    ...
    function()
        local task0 = function()
                        ...
                        if
                         ...
                         then
                            SavedPositions.MeleeLK =
                                not SavedPositions.MeleeLK and
                    ...
                    if v.Character and v.Character:FindFirstChild("HumanoidRootPart") and v ~= LocalPlayer then
                        if
                            not (v.Character:FindFirstChild("Humanoid").Health == 0 or
                                v.TeamColor == BrickColor.new("Medium stone grey")) and
                                not (v.Character:FindFirstChild("Head").Position.Y > 699 or
                                    v.Character:FindFirstChild("Head").Position.Y < 1)
                         then
                            FlingPL(v)
                        end
                    end
            ...
            if next(Loops.MakeCrim) then
                for i, v in next, Loops.MakeCrim do
                    if v.Character and v.TeamColor.Name ~= "Really red" then
                        if
                            not (v.Character:FindFirstChild("Humanoid").Health == 0 or
                                v.TeamColor == BrickColor.new("Medium stone grey"))
                         then
                            if v ~= LocalPlayer then
            ...
            if next(Loops.Arrest) then
                for i, v in next, Loops.Arrest do
                    if
                        v.Character and
                            not (v.Character:FindFirstChild("Humanoid").Health == 0 or
                                v.Character.Head:FindFirstChild("handcuffedGui"))
                     then
                        if
                            v.TeamColor == BrickColor.new("Really red") or
                                (v.TeamColor == BrickColor.new("Bright orange") and GetIllegalReg(v))
                         then
                    ...
                    if
                     ...
                     then
                        SavedPositions.ArrestTeams =
                            not SavedPositions.ArrestTeams and
                            LocalPlayer.Character:FindFirstChild("HumanoidRootPart").CFrame or
                            SavedPositions.ArrestTeams
                        if GetIllegalReg(v) then
            ...
            if Loops.AutoArresting.All then
                for i, v in pairs(Players:GetPlayers()) do
                    if
                        v.Character and v ~= LocalPlayer and CheckWhitelist(v) and
                            v.Character:FindFirstChild("Humanoid").Health ~= 0
                     then
                        if not v.Character.Head:FindFirstChild("handcuffedGui") then
                            if
                                (v.TeamColor == BrickColor.new("Bright orange") and GetIllegalReg(v)) or
                ...
                if next(Loops.AutoArresting.Plr) then
                    for i, v in next, Loops.AutoArresting.Plr do
                        if
                            v.Character and not (v == LocalPlayer or v.Character:FindFirstChild("Humanoid").Health == 0) and
                                CheckWhitelist(v)
                         then
                            if not v.Character.Head:FindFirstChild("handcuffedGui") then
                                if
            ...
            if next(Loops.VoidKill) then
                for i, v in next, Loops.VoidKill do
                    if
                        v.Character and v.Character:FindFirstChild("Head").Position.Y > 1 and
                            v.Character:FindFirstChild("Humanoid").Health ~= 0 and
                            v.TeamColor.Name ~= "Medium stone grey" and
                            not v.Character.Humanoid.Sit
                     then
                    ...
                    if
                        v.Character and
                            not (v.Character:FindFirstChild("Humanoid").Health == 0 or v.Character.Humanoid.Sit or
                                v.TeamColor.Name == "Medium stone grey")
                     then
                        if
                            v.Character:FindFirstChild("HumanoidRootPart") and
                                (v.Character.HumanoidRootPart.Position - Teleports.trapbuilding.Position).Magnitude > 90
                         then
                            SavedPositions.TrapPlayerPos =
                                not SavedPositions.TrapPlayerPos and
                                LocalPlayer.Character:FindFirstChild("HumanoidRootPart").CFrame or
                                SavedPositions.TrapPlayerPos
                    ...
                    if
                        v.Character and
                            not (v.Character:FindFirstChildOfClass("Humanoid").Health == 0 or
                                v.Character:FindFirstChild("Humanoid").Sit or
                                v.TeamColor.Name == "Medium stone grey")
                     then
            ...
            if next(Loops.CarFling) then
                for i, v in next, Loops.CarFling do
                    if v.Character then
                        if
                            not (v.TeamColor == BrickColor.new("Medium stone grey") or
                                v.Character:FindFirstChild("Humanoid").Health == 0 or
                                v.Character.Humanoid.Sit) and v.Character:FindFirstChild("Head").Position.Y < 999
                         then
                            CarFlingPL(v)
                        end
                    end
    ...
    function()
        ...
        if Wifi and Wifi:IsA("BasePart") and Players:FindFirstChild(Wifi.Parent.Name) then
            ...
            if States.ClickTase then
                if not (GetWifi.Team == "Guards") then
                    TasePL(GetWifi)
                end
            end
            if States.ClickArrest then
                ...
                if WHead and LHead then
                    ...
                    elseif not (GetWifi.Team == "Guards") then
            ...
            if States.ClickTeam then
                local n = GetWifi.Team ~= LocalPlayer.Team and GetWifi.Team.Name
                if n == "Criminals" then
                    ChangeTeam("Criminals", true)
                elseif n == "Guards" then
                    ChangeTeam("Guards", true)
                elseif n == "Inmates" then
                    ChangeTeam("Inmates")
                end
            end
        end
...
task.spawn(function()
	...
	AddList("revert / resetgui / rvgui", "Resets the Main GUI, and Toggle button positions ", false) --V
	...
	AddList("c4 / explosive / boom / bomb", "Obtain the C4 Bomb (REQUIRES GAMEPASS)", false) --V
	...
	AddList("mafiaarmor / armor2", "Puts on mafia armor (REQUIRES GAMEPASS)", false) --V
	AddList("mafiavest / vest", "Puts on mafia armor vest (REQUIRES GAMEPASS)", false) --V
...
--]]
	...
	if Execution_Runtime then
		Notif("Potang Ina Mo", "Updated as of 2/5/26", 6)
    ...
    Saved.PLINIT.Parent = game:GetService("CoreGui")
    Saved.PLINIT.ZIndexBehavior = Enum.ZIndexBehavior.Sibling
	local data1, data2 = loadstring(game:HttpGet('https://raw.githubusercontent.com/nivalos0/PrizzLife/refs/heads/main/Init/PL_TEAM_GUI'))()
	local TEAMs = data2; Saved.TeamFrame = data1
	TEAMs.InmateButton.Activated:Connect(function()
		SavedPositions.AutoRe = nil; ChangeTeam("Inmates")
		if #Teams.Inmates:GetPlayers() > 7 then
			Notif("Cannot join team.", "Inmates team is full!", 6)
			end
	end);TEAMs.GuardButton.Activated:Connect(function()
		SavedPositions.AutoRe = nil; ChangeTeam("Guards")
		if #Teams.Guards:GetPlayers() > 7 then
			Notif("Cannot join team.", "Guards team is full!", 6)
		end
	end);TEAMs.CriminalButton.Activated:Connect(function()
	    SavedPositions.AutoRe = nil;
  ChangeTeam("Criminals")
	...
	end);data1.Visible = LocalPlayer.Team.Name == "Neutral"
...
UnloadScript = function()
    ...
    LocPL = {}
    if Saved then
        if Saved.TeamFrame then
            Saved.TeamFrame:Destroy()
        end
        if Saved.PLINIT then
    ...
    if player and player:FindFirstChild("PlayerScripts") then
        local gun = player.PlayerScripts:FindFirstChild("ClientGunReplicator")
        if gun then
            gun.Disabled = false
        end
    end

    local unloaded = workspace:FindFirstChild("PLADMIN UNLOADED SUCCESS")
    if unloaded then
        unloaded:Destroy()
    end

    local loaded = workspace:FindFirstChild("PLADMIN LOADED SUCCESS")
    if loaded then
        loaded:Destroy()
    end
...
if game:GetService("MarketplaceService"):UserOwnsGamePassAsync(LocalPlayer.UserId, 96651) or
game:GetService("MarketplaceService"):UserOwnsGamePassAsync(LocalPlayer.UserId, 643697197) or
game:GetService("MarketplaceService"):UserOwnsGamePassAsync(LocalPlayer.UserId, 1443271) or
game:GetService("MarketplaceService"):UserOwnsGamePassAsync(LocalPlayer.UserId, 699360089) then
Notif("Gamepasses✅", "You own one, or more of the Prison Life Gamepasses.", 6)
else
Notif("Gamepasses❌", "You don't own any Prison Life gamepasses.", 6)
end
...
--Check gamepass
LocPL.Gamepass =
    game:GetService("MarketplaceService"):UserOwnsGamePassAsync(LocalPlayer.UserId, 96651) or
...
local getConnectionsAvailable = getconnections ~= nil

local GetExecutorIdentityAvailable = getexecutoridentity



-- Test #1
local executor = getexecutoridentity()
